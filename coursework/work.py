...
def menu():
    ...
    while len(accounts) == 0 and accountType == 1:
        print()
        accountType = int(
            input('Error! You do not have an account! So sign up or use a guest account\nPlease enter your decision: '))
...
def payMent():
    ...
    coupon = input(
        'Do you have a coupon code? If not then press A. (Mr. Hughes please view the comments for the codes)\nIf yes then enter the code here: ')
    ...
    print(
        'Your credit card has been charged the above total. The charge will be from ByTowne Cineplex. Enjoy your viewing experience!')
    input('')
    # Clearing the screen
    os.system('cls')
    main()
